## mirTarBase WikiPathways Targets Strong Table
#
# index: mirtarbase
# col1: WPID targets
...
exportHash2Csv <- function(h1){
    ...
    write.csv(e, file ="mirt_wptargets_strong_hash.csv",row.names=FALSE)
}

######
## RUN

all_list<-lapply(values(mirt_entreztargets_strong_hash), lookupEntWp)
non_null_list<-all_list[!sapply(all_list,is.null)]
mirt_wptargets_strong_hash<-hash(non_null_list)

exportHash2Csv(mirt_wptargets_strong_hash)
save(mirt_wptargets_strong_hash, file="mirt_wptargets_strong_hash.robj")
